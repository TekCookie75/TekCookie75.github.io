...
$$
similarity(X, Y) := dot(X,Y) / (|X| \cdot |Y|)
$$
...
## Learning the Knowledge (Attention and MLP Clusters)
...
$$
Q = X \cdot W_q    ∈ ℝ^(N × d_head)   — what each token is looking for
K = X \cdot W_k    ∈ ℝ^(N × d_head)   — what each token advertises
V = X \cdot W_v    ∈ ℝ^(N × d_head)   — what each token offers to share
$$
